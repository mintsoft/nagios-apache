...
$def[1]  = rrd::def('var0', $RRDFILE[1], $DS[1], 'AVERAGE');
...
$def[2]  = rrd::def('var0', $RRDFILE[2], $DS[2], 'AVERAGE');
...
for($i=0; $i<count($scoreboard) - ($include_open_slot ? 0 : 1); $i++) {
    $def[4] .= rrd::def("var$i", $RRDFILE[$i+5], $DS[$i+5], 'AVERAGE');
    if ($i == '1')
